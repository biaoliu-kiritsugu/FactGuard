...
from factguard_generation import env
...
class LongAlpaca(Dataset):
    ...
    def examples(self) -> Iterable[Dict[str, Any]]:
        ...
        paths = [
            env.FACTGUARD_DATA_DIR.joinpath("docs", "LongAlpaca-12k", "LongAlpaca-12k.json"),
            env.FACTGUARD_DATA_DIR.joinpath(
...
class AncientBook(Dataset):
    ...
    def examples(self) -> Iterable[Example]:
        for example in jsonlines.open(
            env.FACTGUARD_DATA_DIR / "docs" / "chinese_book.jsonl", "r"
        ):
            yield Example(example["text"])
        for filepath in env.FACTGUARD_DATA_DIR.joinpath("docs", "shu", "books").glob(
            "**/*.txt"
        ):
...
class Gutenberg(Dataset):
    ...
    def examples(self):
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs", "gutenberg_english")
...
class PileOfLaw(Dataset):
    ...
    def examples(self):
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs", "pile-of-law")
        for example in jsonlines.open(data_dir / "train.jsonl", "r"):
            yield Example(example["text"])
...
class ChineseLaw(Dataset):
    ...
    def examples(self):
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs")
        for example in jsonlines.open(
...
class LongDataSkyPile(Dataset):
    ...
    def examples(self) -> Iterable[Example]:
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs", "LongData-Corpus", "LongData_zh")
        for example in jsonlines.open(
...
class LongDataEnWiki(Dataset):
    ...
    def examples(self) -> Iterable[Example]:
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs", "LongData-Corpus", "LongData_en")
        for example in jsonlines.open(
...
class LongBench(Dataset):
    ...
    def examples(self) -> Iterable[Example]:
        data_dir = env.FACTGUARD_DATA_DIR.joinpath("docs", "LongBench", "data")
