...
def read_original_examples(filename):
    examples = dict()
    for example in jsonlines.open(
        MISA_DATA_DIR.joinpath("generation", "misattr_bak", filename)
    ):
        if "log" in example:
            continue
...
def read_new_examples(filename):
    examples = dict()
    for example in jsonlines.open(MISA_DATA_DIR.joinpath("generation", filename), "r"):
        if "log" in example:
            continue
        examples[example["uid"]] = example
...
for filename in [
    ...
    writer = jsonlines.open(MISA_DATA_DIR / "misattr_merged" / filename, "w")
